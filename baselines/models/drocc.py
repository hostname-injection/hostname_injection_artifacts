...
class DROCCBaseline(BaselineModel):
    def __init__(
        self,
        *,
        ...
        if self.use_official_repo and self.repo_root:
            ...
            ensure_repo("drocc", Path(self.repo_root), allow_downloads=self.allow_downloads)
    ...
    def fit(self, texts: Sequence[str], labels: Sequence[int]) -> None:
        ...
        # Use benign embeddings for adversarial augmentation
        ...
        for _ in range(self.epochs):
            for batch_x, batch_y in loader:
                ...
                # Generate synthetic adversarial negatives around benign points
                benign_mask = batch_y == 0
                if benign_mask.any():
                    noise = torch.randn_like(batch_x) * self.noise_radius
                    adv = batch_x[benign_mask] + noise[benign_mask]
                    adv_labels = torch.ones(adv.shape[0], device=self.device)
                    logits_adv = self._net(adv).squeeze(1)
                    loss_adv = criterion(logits_adv, adv_labels)
                else:
                    loss_adv = 0.0

                logits = self._net(batch_x).squeeze(1)
                loss_main = criterion(logits, batch_y)
        ...
        # Threshold based on benign scores
        self._net.eval()
        with torch.no_grad():
            benign_t = torch.tensor(benign, dtype=torch.float32, device=self.device)
            scores = torch.sigmoid(self._net(benign_t)).squeeze(1).cpu().numpy()
    ...
    def _score(self, embeddings: np.ndarray) -> np.ndarray:
        ...
        with torch.no_grad():
            ...
            scores = torch.sigmoid(self._net(x)).squeeze(1).cpu().numpy()
        return scores
