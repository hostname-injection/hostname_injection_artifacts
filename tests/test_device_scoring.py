...
def test_torch_scoring_normalizes_nonunit_embeddings():
    try:
        import torch
    except Exception:
        return

    cone = ConeConfig(dim=4, num_cones=16, active_cones=4, use_lsh=False, seed=13)
    config = CCDConfig(cone=cone)
    cones = ConePartition.build(cone)
    ...
    scores_unit = model.score_embeddings(unit_embeddings)
    scores_scaled_torch = model.score_embeddings(torch.tensor(scaled_embeddings))

    np.testing.assert_allclose(scores_unit, scores_scaled_torch, rtol=1e-5, atol=1e-5)
