...
def test_archive_excludes_intermediate_training_checkpoints(tmp_path: Path) -> None:
    ...
    bundled_model_dir = tmp_path / "caho_model_checkpoint"
    bundled_model_dir.mkdir()
    (bundled_model_dir / "model.safetensors").write_text("release model", encoding="utf-8")
    ...
    assert "caho_model_checkpoint" in result["top_level_bytes"]
