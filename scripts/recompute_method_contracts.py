...
def validate_caho_support(expected: Mapping[str, Any]) -> dict[str, Any]:
    ...
    invalid_binary = [
        ...
        {"contrastive_loss_weight": math.nan},
        {"binary_hidden_dim": 0},
        {"checkpoint_every_steps": -1},
    ]
    for override in invalid_contrastive:
        try:
            BenchmarkContrastiveTrainer(object(), **{**contrastive_valid, **override})
        except ValueError:
            pass
        else:
            raise ValueError(f"BenchmarkContrastiveTrainer accepted invalid hyperparameters: {override!r}")
    for override in invalid_binary:
        try:
            BenchmarkBinaryContrastiveTrainer(object(), **{**binary_valid, **override})
        except ValueError:
            pass
        else:
            raise ValueError(f"BenchmarkBinaryContrastiveTrainer accepted invalid hyperparameters: {override!r}")
    ...
    return {
        ...
        "training_hyperparameter_validation_available": True,
